...
BeforeAll {
    ...
    # ---------------------------------------------------------------------------
    Import-Module $script:HarnessPath -Force -DisableNameChecking -ErrorAction SilentlyContinue
    ...
    function script:Invoke-InSta {
        ...
        Invoke-WsusStaHarness -ScriptBlock $ScriptBlock -ModulePaths @($script:ModulePath)
    }
}
    ...
    Context 'Window with Owner' -Skip:(-not $script:WpfAvailable) {
        It 'Uses CenterOwner when Owner is provided' {
            $result = Invoke-InSta -ScriptBlock {
                ...
                $owner.Show()
                try { (New-WsusDialog -Title 'Child' -Owner $owner).Window.WindowStartupLocation.ToString() }
                finally { $owner.Close() }
            }
        ...
        It 'Owner property is assigned' {
            $result = Invoke-InSta -ScriptBlock {
                $owner = [System.Windows.Window]::new()
                $owner.Show()
                try { $null -ne (New-WsusDialog -Title 'Child' -Owner $owner).Window.Owner }
                finally { $owner.Close() }
            }
            $result | Should -Be $true
        }
