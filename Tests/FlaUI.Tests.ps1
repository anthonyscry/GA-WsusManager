...
BeforeDiscovery {
    ...
    $script:ExePath = Resolve-WsusGuiExecutablePath -RepoRoot $script:RepoRoot

    $harnessPath = Join-Path $PSScriptRoot "FlaUITestHarness\FlaUITestHarness.psm1"
    $script:FlaUIAvailable = Test-Path $harnessPath
    $script:FlaUIAssembliesLoaded = Test-WsusFlaUIAssembliesAvailable -HarnessPath $harnessPath
...
BeforeAll {
    ...
    New-WsusTestDirectory -Path $script:ScreenshotDir | Out-Null

    # Kill any existing instances
    Stop-WsusTestProcess -Name $script:AppName -Force
...
Describe "WSUS Manager Startup" -Skip:(-not $script:CanRunTests) {
    ...
    AfterAll {
        if ($script:AppContext) {
            try { Save-UIScreenshot -Path (Join-Path $script:ScreenshotDir "Startup_Final.png") } catch { Write-Verbose "Final screenshot skipped: $($_.Exception.Message)" }
        }
        Stop-GuiApplication -Force -ErrorAction SilentlyContinue
    }
...
Describe "WSUS Manager Navigation" -Tag "Navigation" -Skip:(-not $script:CanRunTests) {
    ...
    AfterAll {
        try { Save-UIScreenshot -Path (Join-Path $script:ScreenshotDir "Navigation_Final.png") } catch { Write-Verbose "Final screenshot skipped: $($_.Exception.Message)" }
        Stop-GuiApplication -Force -ErrorAction SilentlyContinue
    }
            ...
            if ($settingsDialog.PSObject.Properties['_ComElement']) {
                ...
                try {
                    ...
                    try {
                        ...
                        Start-Sleep -Milliseconds 500
                    } catch { Write-Verbose "ESC fallback failed: $($_.Exception.Message)" }
                }
            }
...
Describe "WSUS Manager Performance" -Tag "Performance" -Skip:(-not $script:CanRunTests) {
    Context 'Startup Time' {
        It 'Application starts within 45 seconds' {
            $sw = [System.Diagnostics.Stopwatch]::StartNew()
            Start-GuiApplication -Path $script:ExePath -Timeout 60 | Out-Null
            $sw.Stop()
...
Describe "WSUS Manager Resilience" -Tag "Resilience" -Skip:(-not $script:CanRunTests) {
    ...
    AfterAll {
        try { Save-UIScreenshot -Path (Join-Path $script:ScreenshotDir "Resilience_Final.png") } catch { Write-Verbose "Final screenshot skipped: $($_.Exception.Message)" }
        Stop-GuiApplication -Force -ErrorAction SilentlyContinue
    }
        ...
        It 'Survives opening and closing Settings multiple times' {
            for ($i = 0; $i -lt 3; $i++) {
                ...
                if ($sw -and $sw.PSObject.Properties['_ComElement']) {
                    ...
                    } catch { Write-Verbose "Window close attempt failed: $($_.Exception.Message)" }
                }
...
Describe "WSUS Manager WSUS-Dependent Buttons" -Tag "Dialogs" -Skip:(-not $script:CanRunTests) {
    ...
    Context 'WSUS-required buttons are disabled on clean VM' {
        # On a clean VM without WSUS, these buttons are disabled with Opacity=0.5
        ...
        It "WSUS button '<_>' is disabled on clean VM" -TestCases $wsusButtons {
            param($_)
            $btn = Find-UIElement -AppContext $script:AppContext -AutomationId $_ -Timeout 5
            $btn | Should -Not -BeNullOrEmpty -Because "Button $_ should exist"
            $btn.IsEnabled | Should -BeFalse -Because "WSUS button $_ should be disabled on clean VM"
        }
    }
}
...
Describe "WSUS Manager Non-WSUS Buttons" -Tag "Operations" -Skip:(-not $script:CanRunTests) {
    ...
    Context 'Non-WSUS buttons are enabled on clean VM' {
        # These buttons work without WSUS installed and should always be enabled
        $enabledButtons = @(
            "BtnDashboard", "BtnInstall", "BtnHistory", "BtnHelp",
            "BtnSettings", "BtnAbout"
        )

        It "Non-WSUS button '<_>' is enabled on clean VM" -TestCases $enabledButtons {
            param($_)
            $btn = Find-UIElement -AppContext $script:AppContext -AutomationId $_ -Timeout 5
            $btn | Should -Not -BeNullOrEmpty -Because "Button $_ should exist"
            $btn.IsEnabled | Should -BeTrue -Because "Non-WSUS button $_ should be enabled on clean VM"
        }
    }
}
