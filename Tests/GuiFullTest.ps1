...
[CmdletBinding()]
param(
    [string]$AppRoot = "C:\WSUS\WsusManager",
    [string]$ResultFile
)

$ErrorActionPreference = "Continue"
$harnessPath = Join-Path $PSScriptRoot '..\Modules\WsusTestHarness.psm1'
Import-Module $harnessPath -Force -DisableNameChecking -WarningAction SilentlyContinue

if ([string]::IsNullOrWhiteSpace($AppRoot)) {
    $AppRoot = "C:\WSUS\WsusManager"
}
if ([string]::IsNullOrWhiteSpace($ResultFile)) {
    $ResultFile = Join-Path $AppRoot 'gui-fulltest-result.txt'
}
...
try {
    ...
    function Get-MainWindow($timeout = 30) {
        ...
        while ($sw.Elapsed.TotalSeconds -lt $timeout) {
            try {
                ...
                if ($mw) { return $mw }
            } catch { Write-Verbose "MainWindow probe: $($_.Exception.Message)" }
    ...
    function Find-El($parent, $automationId, $timeout = 10) {
        ...
        while ($sw.Elapsed.TotalSeconds -lt $timeout) {
            ...
            } catch { Write-Verbose "Element probe: $($_.Exception.Message)" }
    ...
    function Invoke-ElementClick($element) {
        ...
        try {
            ...
            } catch { Write-Verbose "InvokePattern probe: $($_.Exception.Message)" }
            return $false
        }
    }
    ...
    function Open-Dashboard {
        ...
        if ($mw) { $b = Find-El $mw "BtnDashboard" -timeout 5; if ($b) { Invoke-ElementClick $b } }
    ...
    Stop-WsusTestProcess -Name GA-WsusManager -Force
    Start-Sleep -Seconds 2

    Out-Log "[SETUP] Launching WSUS Manager..."
    $exePath = Join-Path $AppRoot 'GA-WsusManager.exe'
    $proc = Start-Process -FilePath $exePath -WorkingDirectory $AppRoot -PassThru
    ...
    @(
        ...
        if ($null -eq $btn) { TF "Nav: $($_.D)" "Button not found"; Open-Dashboard | Out-Null; continue }
        ...
        if (-not (Invoke-ElementClick $btn)) { TF "Nav: $($_.D)" "Click failed"; Open-Dashboard | Out-Null; continue }
    ...
    $mw = Get-MainWindow -timeout 10
    Invoke-ElementClick (Find-El $mw "BtnAbout" -timeout 5) | Out-Null
    Start-Sleep -Milliseconds 800
    ...
    if ($hb -and (Invoke-ElementClick $hb)) {
        Start-Sleep -Milliseconds 800
        $mw = Get-MainWindow -timeout 10
    ...
    $mw = Get-MainWindow -timeout 10
    Invoke-ElementClick (Find-El $mw "BtnInstall" -timeout 5) | Out-Null
    Start-Sleep -Milliseconds 800
