...
Describe "WsusExport Module" {
    Context "Module Loading" {
        ...
        It "Should export New-WsusTransferPlan function" {
            Get-Command New-WsusTransferPlan -Module WsusExport | Should -Not -BeNullOrEmpty
        }
...
Describe "Export-WsusContent" {
    Context "Parameter validation" {
        ...
        It "Should forward MaxAgeDays to robocopy" {
            Mock Invoke-WsusRobocopy { @{ Success = $true; Message = "" } } -ModuleName WsusExport
            Mock Test-Path { $true } -ModuleName WsusExport
            Mock Get-ChildItem { @() } -ModuleName WsusExport

            $null = Export-WsusContent -SourcePath "C:\WSUS" -DestinationPath "C:\Export" -MaxAgeDays 14
