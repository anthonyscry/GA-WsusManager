...
Describe "New-WsusMaintenanceTask" {
    ...
    Context "With valid parameters" {
        BeforeAll {
            ...
            Mock Read-Host { $null } -ModuleName WsusScheduledTask
        }

        It "Should return a hashtable" {
            $result = New-WsusMaintenanceTask -Schedule Weekly -DayOfMonth 15 -Time "03:00" -RunAsUser "SYSTEM"
            $result | Should -BeOfType [hashtable]
        }
    ...
    Context "With Monthly schedule" {
        BeforeAll {
            Mock Get-ScheduledTask { $null } -ModuleName WsusScheduledTask
            Mock Register-ScheduledTask {
                param($TaskName, $Xml, $User, $LogonType, $RunLevel, $AccountPwd)
                [PSCustomObject]@{ TaskName = $TaskName; Xml = $Xml; Registered = $true }
            } -ModuleName WsusScheduledTask
            Mock New-ScheduledTaskTrigger { [PSCustomObject]@{} } -ModuleName WsusScheduledTask
            Mock New-ScheduledTask { [PSCustomObject]@{ Triggers = @([PSCustomObject]@{}) } } -ModuleName WsusScheduledTask
            Mock Test-Path { $true } -ModuleName WsusScheduledTask
            Mock Read-Host { $null } -ModuleName WsusScheduledTask
        }

        It "Should use XML registration path for monthly schedule" {
            $result = New-WsusMaintenanceTask -Schedule Monthly -DayOfMonth 15 -Time "03:00" -RunAsUser "SYSTEM"
            $result | Should -BeOfType [hashtable]
        }

        It "Should include ScheduleByMonth in XML for monthly schedule" {
            $result = New-WsusMaintenanceTask -Schedule Monthly -DayOfMonth 15 -Time "03:00" -RunAsUser "SYSTEM"
            # The result should be a hashtable with the test path indicator.
            # We verify the function returned without throwing on the XML path.
