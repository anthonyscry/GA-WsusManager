...
- Firewall rules
- Directory permissions (including `Authenticated Users` read access on `C:\WSUS`)
- IIS `/Content` physical path to `C:\WSUS\WsusContent`
- SSL configuration
...
**Solution:**
- WSUS root/content path must be `C:\WSUS`
- IIS `WSUS Administration/Content` must point to `C:\WSUS\WsusContent`
- NOT `C:\WSUS\wsuscontent` as the WSUS root
...
1. **Use Reset Content button** (Recommended)
   - In WSUS Manager, click the **Reset Content** button in the DIAGNOSTICS sidebar section
   ...
   - After it completes, wait about **5-10 minutes** for WSUS to register the files
...
3. **Verify content path and permissions**
   - WSUS root/content path should be `C:\WSUS`
   - IIS `WSUS Administration/Content` should point to `C:\WSUS\WsusContent`
   - Ensure you imported update files into `C:\WSUS\WsusContent`
   - Confirm `Authenticated Users` has read access on `C:\WSUS`

**Note:** The install workflow already performs the normal post-install content step. Use **Reset Content** after import only when content state is clearly stuck.

**Common repair finding:**
- If clients can scan but not download, Diagnostics should report one of these:
  - missing `Authenticated Users` read access on `C:\WSUS`
  - IIS `/Content` pointing somewhere other than `C:\WSUS\WsusContent`
