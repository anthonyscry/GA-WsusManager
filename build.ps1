...
.PARAMETER NoPush
    Backward-compatible switch. Git publishing is disabled by default.

.PARAMETER Push
    Commit and push dist artifacts after a successful build. Use only for an explicit release publish.
...
if (Test-Path $metadataPath) {
    try {
        ...
        if ($meta.version) { $Version = ([string]$meta.version).Trim() }
    } catch {
        Write-Warning "Could not read metadata.json: $($_.Exception.Message)"
    }
}
...
"@
        ...
        # Git operations are opt-in for release safety. Building should not mutate
        # repository history or push artifacts unless the caller explicitly asks.
        if ($Push -and -not $NoPush) {
            Write-Host "`n[*] Committing to git (-Push specified)..." -ForegroundColor Yellow
            ...
            Write-Host "`n[*] Skipping git operations (use -Push to publish dist artifacts)" -ForegroundColor Gray
        }
        ...
        Write-Host "  .\dist\                  # Local distribution copy" -ForegroundColor White
        Write-Host "`nGit publish skipped by default. Re-run with -Push only for an explicit release publish." -ForegroundColor Gray
    }
    else {
