...
## 3.5 Evidence-First Validation

For this codebase, the primary automated evidence is now:

- startup probe JSON from `Tests/StartupE2E.Tests.ps1`
- GUI harness result logs from `Tests/GuiFullTest.ps1`
- app/service logs under `C:\\WSUS\\Logs`
- WSUS/report artifacts such as `WSUS_REPORT_PATH`

Treat screenshots as optional supporting evidence, not the primary pass/fail surface.

Recommended pattern:

```powershell
$evidenceRoot = New-WsusTestEvidenceRoot -Prefix 'WsusStartupProbe'
$resultFile = New-WsusTestArtifactPath -RootPath $evidenceRoot -FileName 'startup-probe.json'
...
- `totalPopupCount`
- `errorPopupCount`
- harness/result text artifacts

Only reach for screenshots when visual rendering itself is the question.
...
```powershell
# GUI side: set env var, pass only the variable name in the command string
$env:WSUS_INSTALL_SA_PASSWORD = $saPassword
$command = "& '$script' -SaPasswordEnvVar WSUS_INSTALL_SA_PASSWORD -NonInteractive"
# The secret value stays out of the child process command line.
```
