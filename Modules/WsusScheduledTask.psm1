...
function New-WsusMaintenanceTask {
    ...
    try {
        ...
        # NOTE: Windows PowerShell 5.1's New-ScheduledTaskTrigger does not
        # have a -Monthly switch (added in PowerShell 7+). For Monthly
        # schedules we build a complete task XML and use
        # Register-ScheduledTask -Xml which IS supported in PS 5.1 and
        # honors ScheduleByMonth/DaysOfMonth.
        # Determine day-of-month and time components once.
        $timeParts = $Time -split ':'
        $hour = [int]$timeParts[0]
        $minute = [int]$timeParts[1]
        $monthlyDay = $DayOfMonth

        $isMonthly = ($Schedule -eq 'Monthly')
        $useXmlRegistration = $isMonthly

        if ($useXmlRegistration) {
            # Build a complete task XML with ScheduleByMonth. The
            # settings mirror what New-ScheduledTaskSettingsSet would
            # produce for the non-Monthly case.
            $taskXml = @"
<?xml version="1.0" encoding="UTF-16"?>
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>WSUS monthly maintenance task</Description>
  </RegistrationInfo>
  <Triggers>
    <CalendarTrigger>
      <StartBoundary>2025-01-$(('{0:D2}' -f $monthlyDay))T$(('{0:D2}' -f $hour)):$(('{0:D2}' -f $minute)):00</StartBoundary>
  ...
  <Actions>
    <Exec>
      ...
      <Arguments>-NoProfile -ExecutionPolicy Bypass -File "`$ScriptPath`" -Unattended -Profile `$MaintenanceProfile</Arguments>
    </Exec>
  </Actions>
</Task>
"@
            # Replace placeholders with actual values.
            $taskXml = $taskXml.Replace('`$ScriptPath', $ScriptPath)
            $taskXml = $taskXml.Replace('`$MaintenanceProfile', $MaintenanceProfile)
        } else {
        ...
        if ($useXmlRegistration) {
            # Register via XML for monthly schedules.
