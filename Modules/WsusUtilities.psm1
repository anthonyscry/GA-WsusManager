...
function Invoke-WsusSqlcmd {
    ...
    # Fallback: use sqlcmd.exe (ODBC) when SqlServer/SQLPS module is not
    # available. Do not pass SQL credentials to sqlcmd.exe: the -P argument
    # exposes the password to process inspection. SQL authentication requires
    # the SqlServer PowerShell module so the credential can be supplied through
    # the cmdlet parameter binder instead of the process command line.
    if ($Credential) {
        throw "SQL authentication requires the SqlServer PowerShell module. Refusing sqlcmd.exe fallback because it would expose the password on the process command line."
    }
    ...
    $sqlcmdArgs = @("-S", $ServerInstance, "-d", $Database, "-Q", $Query, "-W", "-b", "-E")
    if ($Variable) {
