...
function Test-WsusContentPermissions {
    ...
    $requiredPermissions = @(
        ...
        @{ Identity = "NETWORK SERVICE"; Rights = [System.Security.AccessControl.FileSystemRights]::FullControl },
        @{ Identity = "NT AUTHORITY\LOCAL SERVICE"; Rights = [System.Security.AccessControl.FileSystemRights]::FullControl },
        @{ Identity = "BUILTIN\IIS_IUSRS"; Rights = [System.Security.AccessControl.FileSystemRights]::ReadAndExecute },
        @{ Identity = "NT AUTHORITY\Authenticated Users"; Rights = [System.Security.AccessControl.FileSystemRights]::ReadAndExecute }
    )
    ...
    try {
        ...
        foreach ($permission in $requiredPermissions) {
            ...
            $requiredRights = $permission.Rights
            $hasPermission = $false

            foreach ($access in $acl.Access) {
                $identityMatches = ($access.IdentityReference -like "*$account*" -or $access.IdentityReference.Value -eq $account)
                $rightsMatch = (($access.FileSystemRights -band $requiredRights) -eq $requiredRights)
                if ($identityMatches -and $access.AccessControlType -eq 'Allow' -and $rightsMatch) {
                    $hasPermission = $true
