...
function Test-WsusSqlLogin {
    <#
    .SYNOPSIS
        Checks if a Windows SQL login exists.
    .PARAMETER SqlInstance
        SQL Server instance name (default: .\SQLEXPRESS)
    .PARAMETER LoginName
        Windows principal name (default: NT AUTHORITY\NETWORK SERVICE)
    .OUTPUTS
        Boolean indicating whether the login exists
    #>
    param(
        [string]$SqlInstance = '.\SQLEXPRESS',
        [string]$LoginName = 'NT AUTHORITY\NETWORK SERVICE'
    ...
    $loginSafe = $LoginName -replace "'", "''"
    $result = Invoke-WsusSqlcmd -ServerInstance $SqlInstance -Database master -Query "SELECT COUNT(*) AS cnt FROM sys.server_principals WHERE name=N'$loginSafe'" -QueryTimeout 10
