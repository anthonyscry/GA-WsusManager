    ...
    try {
        ...
        if ($sql -and $sql.Status -eq "Running") {
            ...
            if (Get-Command Invoke-WsusSqlcmd -ErrorAction SilentlyContinue) {
                $r = Invoke-WsusSqlcmd -Query $q -ServerInstance $SqlInstance -Database 'master' -ErrorAction SilentlyContinue
            } else {
...
function Get-WsusDashboardSnapshot {
<#
.SYNOPSIS
    Returns a cached dashboard snapshot through one deep interface.
.DESCRIPTION
    Concentrates dashboard collection, cache lookup, cache update, and failure
    policy. GUI callers should render this snapshot instead of calling each
    dashboard helper and cache function separately.
#>
    [CmdletBinding()]
    param(
        [string]$SqlInstance = ".\SQLEXPRESS",
    ...
    if (-not $ForceRefresh) {
        $cached = Get-WsusDashboardCachedData -SqlInstance $SqlInstance
        if ($cached) {
            return [pscustomobject]@{
                PSTypeName = 'Wsus.DashboardSnapshot'
                Source = 'Cache'
                Data = $cached
                CollectedAt = $cached.CollectedAt
                DataUnavailable = Test-WsusDashboardDataUnavailable
            }
        }
    }

    $data = Get-WsusDashboardData -SqlInstance $SqlInstance -ModulePath $ModulePath
    Set-WsusDashboardCache -Data $data
